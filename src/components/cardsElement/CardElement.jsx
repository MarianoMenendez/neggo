...
export default function CardsElement({product}) {
    const { id, external_code, name, category, price, image, bit_active, bit_new, count} = product
    ...
    return(
      <div style={{ display: 'flex'}}>
        <div style={{ maxWidth: '30%', overflow: 'hidden' }}>
            <img src={imagen} alt={`img of ${name}`} 
            style={{ width: '100%', minWidth: '60px', objectFit: 'cover' }}/>
        </div>
        <Card style={{ width: '10rem', height: '10rem'}} id={id}>
        <Card.Body>
          ...
          <Button variant="primary" value={id} onClick={(e)=> addToOrder(e) }>Agregar al Pedido</Button> :
          <div>
            <InputGroup className="mb-3">
                <Button variant="outline-secondary" value={id} onClick={(e)=> count>0 && dispatch(setQuantityToCart(e.target.value, count-1))}>
                -
                </Button>
                <Form.Control
                ...
                value={count} 
                onChange={(e) => 
                    dispatch(setQuantityToCart(id, e.target.value === "" ? 0 : parseInt(e.target.value)))
                }
                />
                <Button variant="outline-secondary" value={id} onClick={(e)=> dispatch(setQuantityToCart(e.target.value, count+1))}>
                +
                </Button>
            </InputGroup>
