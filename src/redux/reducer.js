...
export default function reducer(state = initialState, action) {
    switch (action.type){
        ...
        case GET_PRODUCTS_BY_NAME:
            ...
            return {...state, products: productsByName, activeFilters: {category: []}}
        //Me mantiene el filtro cuando cambio de pestaña pero no me deja el nombre del producto que busqué, modificar.

        
        case ADD_PRODUCT_TO_CART:
            if(state.cart.findIndex(product=> product.id === action.payload)>=0){
        ...
        case SET_QUANTITY_TO_CART:
            ...
            if(action.payload.quantity === 0){
                productsCart = state.cart.filter(product => product.id !== action.payload.id)
            } else {
        ...
        case SET_CATEGORY_FILTER:
            if(state.activeFilters.category.includes(action.payload)){
                return {...state, activeFilters: {
                    ...state.activeFilters,
                    category: state.activeFilters.category.filter(cat => cat !== action.payload)
                }}
            } else {
                return {...state, activeFilters: {
                    ...state.activeFilters,
