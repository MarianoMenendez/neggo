...
export const getAllProducts = () => {
  return async function (dispatch) {
    //Se lo tengo que pasar al middelware thunk
    try {
      await axios("http://localhost:3001/products/all").then((res) =>
        dispatch({ type: GET_ALL_PRODUCTS, payload: res.data })
      );
    } catch (error) {
      throw alert(
        "Surgió un problema al cargar los países, por favor recargue la página"
      );
    }
  };
};
...
export const getProductsByName = (name) => {
  //tiene que limpiar los otros filtros, el paginado, todo reiniciarlo y guardar el nombre y listado de paises de respuesta
  return async function (dispatch) {
    //Se lo tengo que pasar al middelware thunk
    try {
      await axios(`http://localhost:3001/products?name=${name}`).then((res) =>
        dispatch({ type: GET_PRODUCTS_BY_NAME, payload: res.data })
      );
    } catch (error) {
      throw alert(
        "Surgió un problema al cargar los países, por favor recargue la página"
